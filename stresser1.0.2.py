...
# Было: logger.setLevel(logging.DEBUG)
logger.setLevel(logging.INFO)  # <--- Ставим INFO
...
# Было: file_handler.setLevel(logging.DEBUG)
file_handler.setLevel(logging.INFO) # <--- Ставим INFO
file_handler.setFormatter(formatter)

# Обработчик для консоли
console_handler = logging.StreamHandler(sys.stdout)
# Было: console_handler.setLevel(logging.DEBUG)
console_handler.setLevel(logging.INFO) # <--- Ставим INFO
...
def is_blacklisted(url):
    ...
    for domain in BLACKLIST:
        # Проверяем, содержится ли запрещенный домен в ссылке
...
# Главный админ - создатель ключей
MAIN_KEY_CREATOR = 
...
def is_subscribed(user_id: int) -> tuple:
    """Проверяет подписку: (has_access: bool, type: str, days_left: int)"""
    with db_lock:
...
def worker_attack(host: str, port: int, path: str, method: str, thread_id: int):
    ...
    #log_attack(f"Поток #{thread_id} ({method}) запущен")
    ...
    #log_attack(f"Поток #{thread_id} остановлен")
...
def start_handler(message):
    ...
    has_access, sub_type, days_left = is_subscribed(user_id)
...
def menu_handler(message):
    user_id = message.chat.id
    if not is_subscribed(user_id)[0]:
        bot.reply_to(message, 
            "❌ *Нет активной подписки!*\n\n"
            "Для использования меню нужна активная подписка.\n"
            "Используй /start для активации ключа.",
            parse_mode="Markdown")
        return
...
def activate_key_menu(call):
    ...
    has_access, _, _ = is_subscribed(user_id)
...
def how_to_get_access(call):
    ...
    text = (
        ...
        "1️⃣ *Купите подписку -> *\n"
        "   • 30 дней - временный доступ\n"
        "   • Пожизненная - полный доступ навсегда\n\n"
        "2️⃣ *Получите ключ активации*\n"
        "   Ключ выглядит так: `PERM-5946555648-1234567890-ABCDEF12`\n\n"
        "3️⃣ *Активируйте ключ*\n"
        "   Используйте команду: `/activate <ваш_ключ>`\n\n"
...
def generate_keys_handler(call):
    ...
    keys = create_keys(amount, key_type, MAIN_KEY_CREATOR)
...
def method_callback(call):
    ...
    if not is_subscribed(user_id)[0]:
        bot.answer_callback_query(call.id, 
            "❌ Нет активной подписки!\n\n"
            "Используйте /start для активации ключа.",
            show_alert=True)
        return
    ...
    def make_handler(m):
        def handler(message):
            ...
            if not is_subscribed(user_id)[0]:
                bot.reply_to(message, 
                    "❌ *Нет активной подписки!*\n\n"
                    "Для использования атак нужна подписка.\n"
                    "Используйте /start для активации ключа.",
                    parse_mode="Markdown")
                return
            ...
            try:
                ...
                if threads < 1 or threads > 500:
                    bot.reply_to(message, 
                        ...
                        "Потоков должно быть от 1 до 500.",
                        parse_mode="Markdown")
                    return
            ...
            if not is_subscribed(user_id)[0]:
                bot.reply_to(message, 
                    "❌ *Подписка истекла во время ожидания!*\n\n"
                    "Активируйте новый ключ: /start",
                    parse_mode="Markdown")
                return
...
if __name__ == "__main__":
    ...
    log_success("База данных загружена")
    log_attack("=" * 70)
    log_attack("🔥 STRESSER BOT v4.2 С ПОДПИСКАМИ ЗАПУЩЕН 🔥")
    log_attack("=" * 70)
